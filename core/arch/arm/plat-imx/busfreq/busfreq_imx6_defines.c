 ...
 *          Definition of the struct busfreq_info field to
...
/*
 * Define the offset of the structure busfreq_info
 * defined in the busfreq_imx6.h file
 */
DEFINES
{
	DEFINE(BUSFREQ_INFO_CPU_TYPE,
			offsetof(struct busfreq_info, cpu_type));
	DEFINE(BUSFREQ_INFO_FREQ,
			offsetof(struct busfreq_info, freq));
	DEFINE(BUSFREQ_INFO_DLL_OFF,
			offsetof(struct busfreq_info, dll_off));
	DEFINE(BUSFREQ_INFO_DDR_SETTINGS,
			offsetof(struct busfreq_info, ddr_settings));
	DEFINE(BUSFREQ_INFO_DDR_SETTINGS_SIZE,
			offsetof(struct busfreq_info, ddr_settings_size));
	DEFINE(BUSFREQ_INFO_DDR_CALIBRATION,
			offsetof(struct busfreq_info, ddr_calibration));
	DEFINE(BUSFREQ_INFO_DDR_CALIBRATION_SIZE,
			offsetof(struct busfreq_info, ddr_calibration_size));
	DEFINE(BUSFREQ_INFO_IOMUX_OFFSETS,
			offsetof(struct busfreq_info, iomux_offsets));
	DEFINE(BUSFREQ_INFO_IOMUX_OFFSETS_SIZE,
			offsetof(struct busfreq_info, iomux_offsets_size));
	DEFINE(BUSFREQ_INFO_CCM_BASE,
			offsetof(struct busfreq_info, ccm_base));
	DEFINE(BUSFREQ_INFO_IOMUX_BASE,
			offsetof(struct busfreq_info, iomux_base));
