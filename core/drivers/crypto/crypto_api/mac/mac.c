 ...
 *
 * Brief   Crypto MAC interface implementation to enable HW driver.
 */
#include <assert.h>
#include <drvcrypt.h>
...
TEE_Result drvcrypt_mac_alloc_ctx(struct crypto_mac_ctx **ctx, uint32_t algo)
{
	...
	hw_mac_allocate mac_alloc = NULL;
	...
	else if (algo_id >= TEE_MAIN_ALGO_AES && algo_id <= TEE_MAIN_ALGO_DES3)
