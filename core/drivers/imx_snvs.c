...
static bool is_mks_locked(void)
{
	...
	hp_lr = io_read32(base + SNVS_HPLR);
	lp_lr = io_read32(base + SNVS_LPLR);

	if ((hp_lr & SNVS_HPLR_MKS_SL) || (lp_lr & SNVS_LPLR_MKS_HL))
		return true;
...
/*
 * Set the Master key to use OTPMK and lock it. Return true if the operation is
 * successful. Return false if the master key is already locked.
 */
...
{
	uint32_t hp_status = 0;
	vaddr_t base = core_mmu_get_va(SNVS_BASE, MEM_AREA_IO_SEC, SNVS_SIZE);

	hp_status = io_read32(base + SNVS_HPSR);

	if (hp_status & (SNVS_HPSR_OTPMK_ZERO | SNVS_HPSR_OTPMK_SYND))
		return false;
...
{
	/* The OTPMK can only be set on closed devices */
