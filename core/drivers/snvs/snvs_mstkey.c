...
static inline bool is_otpmk_selected(void)
{
	...
	hp_mks = read32(snvs_base + SNVS_HPCOMR);
	 ...
	 */
	if (hp_mks) {
		lp_mks = read32(snvs_base + SNVS_LPMKCR);
...
static inline bool is_mks_locked(void)
{
	...
	hp_lr = read32(snvs_base + SNVS_HPLR);
	lp_lr = read32(snvs_base + SNVS_LPLR);
...
static inline bool is_otpmk_valid(void)
{
	...
	hp_status = read32(snvs_base + SNVS_HPSTATUS);
