 ...
 */
/*
 * The DCP module provides support for general encryption and hashing functions
 * typically used for security functions. It also provides SRAM memory storage
 * up to four keys. Using this features, the AES-CMAC algorithm it’s
...
/* Virtual DCP base address */
static vaddr_t dcp_base;
/* Driver state: initialized or not */
static bool driver_init;

/* Spinlock used to lock SRAM key register */
unsigned int key_store_spinlock = SPINLOCK_UNLOCK;
uint32_t key_store_except;

/* Spinlock used to lock clock access */
unsigned int clock_spinlock = SPINLOCK_UNLOCK;
uint32_t clock_except;

struct lock_channel {
	struct mutex m; /* Mutex used to lock channels registers */
};

static struct lock_channel lock_channel[DCP_NB_CHANNELS] = {
	{
		/* Channel 0 */
		.m = MUTEX_INITIALIZER,
	},
...
/*
 * Counter used with clock_spinlock
 * It avoids:
 *	- useless clock activation (i.e. when the clock was activated by
 *	  another channel)
 *	- to disable clock whereas clock is needed by another channel
 */
static unsigned int clock_refcount;

static const struct hashalg hash_alg[3] = {
	{
		/* SHA1*/
...
/* Compatible DCP string for i.MX6ULL/ULZ/SL/SLL device tree */
#ifdef CFG_DT
...
/* Memory used for context switching */
static struct align_buf hw_context_buffer;

/*
 * Enable/disable the DCP clock
 *
...
static void dcp_clk_enable(bool enable)
{
	...
	ccm_base = core_mmu_get_va(CCM_BASE, MEM_AREA_IO_SEC);
	reg = io_read32(ccm_base + CCM_CCGR0);

	if (enable)
		reg |= DCP_CLK_ENABLE_MASK;
	else
		reg &= ~DCP_CLK_ENABLE_MASK;
...
/*
 * Use spinlock to secure clock register wrtiting
 * Enable clock if it's not done
 * Disable clock only if no channel is still using it
 *
 * @enable   Enable the clock if true, disable if false
 */
static void secure_dcp_clk_enable(bool enable)
{
	clock_except = cpu_spin_lock_xsave(&clock_spinlock);
	if (enable) {
		clock_refcount++;
		if (clock_refcount == 1)
			dcp_clk_enable(true);
...
static TEE_Result dcp_lock_known_channel(enum dcp_channel chan)
{
	...
	for (timeout = 0; timeout < DCP_MAX_TIMEOUT; timeout++) {
		lock = mutex_trylock(&lock_channel[chan].m);
		if (lock)
			return TEE_SUCCESS;
...
{
	if (lock_channel[chan].m.state)
		mutex_unlock(&lock_channel[chan].m);
}
...
static TEE_Result dcp_lock_any_ready_channel(enum dcp_channel *channel)
{
	...
	for (i = 0; i < DCP_NB_CHANNELS; i++) {
		ret = dcp_lock_known_channel((enum dcp_channel)i);
		if (ret == TEE_SUCCESS) {
			*channel = (enum dcp_channel)i;
			return ret;
		}
	}
 ...
 */
static TEE_Result dcp_lock_channel(struct op_state *op_state,
				   enum dcp_channel *channel)
{
	TEE_Result ret = TEE_ERROR_GENERIC;

	if (op_state->any_channel)
		ret = dcp_lock_any_ready_channel(channel);
	else
		ret = dcp_lock_known_channel(*channel);
...
{
	unsigned int offset = CONTEXT_SW_OFFSET(dcp_data->channel);

	/* Configure restore context descriptor */
	restore_context_desc->ctrl0 =
		DCP_CONTROL0_ENABLE_MEMCOPY | DCP_CONTROL0_DECR_SEMAPHORE |
		DCP_CONTROL0_CHAIN | DCP_CONTROL0_INTERRUPT_ENABLE;
...
TEE_Result dcp_store_key(uint32_t *key, unsigned int index)
{
	...
	/* Lock access to SRAM key registers */
	key_store_except = cpu_spin_lock_xsave(&key_store_spinlock);

	/* Enable clock if it's not done */
	secure_dcp_clk_enable(true);

	val = SRAM_KEY_INDEX(index);
	...
	/* Unlock access to SRAM key registers */
					     ...
					     struct aesdata *aes)
{
	 ...
	 * If msb(k0) = 0, k1 = k0 << 1
	 *    msb(k0) = 1, k1 = (k0 << 1) ^ AES128CMAC_RB_CONST
	 * If msb(k1) = 0, k2 = k1 << 1
	 *    msb(k1) = 1, k2 = (k1 << 1) ^ AES128CMAC_RB_CONST
	 */
	left_shift_buffer(buffer, k0, DCP_AES128_KEY_SIZE);

	if (k0[0] & MSB_INT8)
		buffer[DCP_AES128_KEY_SIZE - 1] ^= AES128CMAC_RB_CONST;

	if (message_size == DCP_AES128_BUFFER_SIZE) {
		memcpy(subkey, buffer, DCP_AES128_KEY_SIZE);
	} else if (message_size < DCP_AES128_BUFFER_SIZE) {
		left_shift_buffer(subkey, buffer, DCP_AES128_KEY_SIZE);
		if (buffer[0] & MSB_INT8)
			subkey[DCP_AES128_KEY_SIZE - 1] ^= AES128CMAC_RB_CONST;
	}
...
void dcp_aes_do_final(struct aesdata *aes)
{
	if (aes) {
		...
		dcp_free(aes->dcp_data.sw_context_buffer.data);
	}
			     ...
			     size_t len)
{
	...
	for (idx = 0; idx < nb_block; idx++) {
		if (!idx && hashdata->block_data_filled) {
			...
			/* Fill the buffer to it's maximum */
			    ...
			    size_t digest_size)
{
	...
	dcp_free(hashdata->dcp_data.sw_context_buffer.data);

exit:
	dcp_free(payload.data);
...
TEE_Result dcp_init(void)
{
	...
	/* Clock initialization (CCM_CCGR0 register) */
	dcp_clk_enable(true);
